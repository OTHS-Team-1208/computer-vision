...
string SystemCalluponCleanProgramExit="sudo poweroff";
bool EnableSystemCall=true;

bool enableAspectRatioTest=false, enableRigidityTest=false;
int AllowableRigidity=5;
double AspectRatioforHotTarget, AspectRatioforNotHotTarget, AspectRatioDevianceforHotTarget, AspectRatioDevianceforNotHotTarget;
//_____________________________________
...
void FindSquares(); //Finds and tests all contours found within an image to see if it is an eligible square
    ...
    bool RectangleShapeRestricter(vector<Point> PotentialTarget); //This function calls the Rigidity or Aspect Ratio Tests from within it
        bool RigidHorizontalorVerticalTest(vector<Point> &PotentialTarget); //determines if rectangles are either horizontal or vertical
        bool AspectRatioTest(const vector<Point> &PotentialTarget); //determines if rectangles have the correct/requested aspect ratio
        void SortVertices(vector<Point> &PotentialTarget); //sorts vertices according to location on picture map
...
//This is the Where the code execution begins ... you should know this if you read the www.cplusplus.com manual to C++ .
//_____________________________________
int main(int argc, char** argv) {
    ...
    for(unsigned int iteration=1 ; iteration!=ProgramIterationLimit && !EndProgram; iteration++) //If either it reaches the iteration limit or if LabView sends the EndProgram command, then the program will shutdown
    {
        ...
        if(enablenetworking) {SendNetworkPacketsThread=thread(SendNetworkPackets, Squares.size(), MilliSecondsPerRenderedFrames, HostPortNumber);}
        ...
        SendNetworkPacketsThread.join();
    }
    sleep(1); //I added this to allow other threads such as the fucntion receivepackets time to finish their execution
    if (EnableSystemCall) std::system(SystemCalluponCleanProgramExit.c_str()); //Execute Specified System Call upon Clean Shutdown of Hot Goal Detection Program (In this case we told it to shut down).
...
void FindSquaresThreader(int startingindex, Mat gray0, Mat gray) { //this function is called multiple times as individual threads
    ...
    for( int l = startingindex+StartingIterationIndex; l < NumberofThresholdingIterations; l+=ThreadNumber) //Basically, in the end, this for loop is what is multithreaded
    {
        ...
        for( size_t i = 0; i < contours.size(); i++ )
        {
               ...
               isContourConvex(Mat(approx)) )
            {
                ...
                if( maxCosine < MaxAllowableAngle ) {
                    if (enableRigidityTest || enableAspectRatioTest) { //only if the Rigidity Test or the Aspect Ratio Test is requested
                        if (RectangleShapeRestricter(approx)) {
                            SquaresTempforMultiThreading[startingindex].push_back(approx);
                        }
                    }
                    else {SquaresTempforMultiThreading[startingindex].push_back(approx);}}
                //write eligible squares to the temporary squares object
...
bool RectangleShapeRestricter(vector<Point> PotentialTarget) {
    SortVertices(PotentialTarget);
    return ((enableAspectRatioTest ? AspectRatioTest(PotentialTarget) : true) && (enableRigidityTest ? RigidHorizontalorVerticalTest(PotentialTarget) : true));
}

bool RigidHorizontalorVerticalTest(vector<Point> &PotentialTarget) {
    int difference[4];
    int count=0;
    difference[0]=abs(PotentialTarget[0].y-PotentialTarget[1].y);
    difference[1]=abs(PotentialTarget[3].y-PotentialTarget[2].y);
    difference[2]=abs(PotentialTarget[1].x-PotentialTarget[2].x);
    difference[3]=abs(PotentialTarget[3].x-PotentialTarget[0].x);
    for(int i=0; i<4; i++) {
        if (difference[i]<AllowableRigidity) count++;
    }
    if (count==4) {return true;}
    else return false;
}

bool AspectRatioTest(const vector<Point> &PotentialTarget) {
    double XLength=0.5*( fabs(PotentialTarget[0].x-PotentialTarget[1].x) + fabs(PotentialTarget[3].x-PotentialTarget[2].x));
    double YLength=0.5*( fabs(PotentialTarget[2].y-PotentialTarget[1].y) + fabs(PotentialTarget[3].y-PotentialTarget[0].y));
    double AspectRatio=XLength/YLength;
    if ((AspectRatio>(AspectRatioforHotTarget-AspectRatioDevianceforHotTarget) && AspectRatio<(AspectRatioforHotTarget+AspectRatioDevianceforHotTarget)) || (AspectRatio>(AspectRatioforNotHotTarget-AspectRatioDevianceforNotHotTarget) && AspectRatio<(AspectRatioforNotHotTarget+AspectRatioDevianceforNotHotTarget))) {
        return true;
    }
    else return false;
}

void SortVertices(vector<Point> &PotentialTarget) {
    int temp_x;
    int temp_y;
    int i, swapped;
    do {
        swapped = 0;
        for (i = 1; i < 4; i++) {
            if (PotentialTarget[i-1].y > PotentialTarget[i].y) {
                temp_x = PotentialTarget[i-1].x;
                temp_y = PotentialTarget[i-1].y;
                PotentialTarget[i-1].x = PotentialTarget[i].x;
                PotentialTarget[i-1].y = PotentialTarget[i].y;
                PotentialTarget[i].x = temp_x;
                PotentialTarget[i].y = temp_y;
                swapped = 1;
            }
        }
    } while (swapped == 1);
    ...
    if (PotentialTarget[0].x > PotentialTarget[1].x) {
        temp_x = PotentialTarget[0].x;
        temp_y = PotentialTarget[0].y;
        PotentialTarget[0].x = PotentialTarget[1].x;
        PotentialTarget[0].y = PotentialTarget[1].y;
        PotentialTarget[1].x = temp_x;
        PotentialTarget[1].y = temp_y;
    }
    // Make sure bottom two points are right to left
    if (PotentialTarget[2].x < PotentialTarget[3].x) {
        temp_x = PotentialTarget[2].x;
        temp_y = PotentialTarget[2].y;
        PotentialTarget[2].x = PotentialTarget[3].x;
        PotentialTarget[2].y = PotentialTarget[3].y;
        PotentialTarget[3].x = temp_x;
        PotentialTarget[3].y = temp_y;
    }
    Point temp1, temp2;
    temp1=PotentialTarget[0];
    temp2=PotentialTarget[1];
    PotentialTarget[0]=temp2;
    PotentialTarget[1]=temp1;
    temp1=PotentialTarget[2];
    temp2=PotentialTarget[3];
    PotentialTarget[2]=temp2;
    PotentialTarget[3]=temp1;
}
...
void DisplayGlobals() { //Self-Explanatory
    ...
    "The End-Of-Program system command is set to: ||"<<SystemCalluponCleanProgramExit<<"|| and is "<<(EnableSystemCall ? "Enabled" : "Disabled")<<"\n\n"<<
    ...
    cout<<"The Number of Threshold Iterations is at +++ "<<NumberofThresholdingIterations<<" AND starts at Index: "<<StartingIterationIndex<<"\n\n";
    
    cout<<"Aspect Ratios are: "<<(enableAspectRatioTest ? "ENABLED" : "Disabled")<<"\n";
    cout<<"The Hot Target Ratio is set to: "<<AspectRatioforHotTarget<<" +- "<<AspectRatioDevianceforHotTarget<<"\n";
    cout<<"The Cold Target Ratio is set to: "<<AspectRatioforNotHotTarget<<" +- "<<AspectRatioDevianceforNotHotTarget<<"\n\n";
    
    cout<<"The Rigidity Test is: "<<(enableRigidityTest ? ("ENABLED with an allowance of " + std::to_string(AllowableRigidity) + " pixels") : "Disabled")<<"\n\n";
...
void ReWriteGlobals(int argc, char** argv){ //Self-Explanatory
    if (argc==2) {
        ...
        settingsfile>>enablenetworking>>TargetIPAddress>>TargetPortNumber>>HostPortNumber>>LabViewKillString>>ReceivePacketsDelay>>textconsole>>GUIMODE>>DemoMode>>DemoThresholdingSleep>>DemoBeginFinishSleep>>TogglePyramidUpDownBlur>>MinAllowableSquareArea>>MaxAllowableSquareArea>>PolygonApproximationAccuracyProportion>>MaxAllowableAngle>>SquareCentersDistanceLimit>>CannyUpperLimitThreshold>>StartingIterationIndex>>NumberofThresholdingIterations>>enableAspectRatioTest>>AspectRatioforHotTarget>>AspectRatioDevianceforHotTarget>>AspectRatioforNotHotTarget>>AspectRatioDevianceforNotHotTarget>>enableRigidityTest>>AllowableRigidity>>ProgramIterationLimit>>EnableSystemCall>>SystemCalluponCleanProgramExit;
        settingsfile.close();
        QstringTargetIPAddress=QString::fromStdString(TargetIPAddress);
        if (DemoMode) {GUIMODE=false;}
            for (int i=0; i<SystemCalluponCleanProgramExit.size(); i++) {
                if (SystemCalluponCleanProgramExit[i]=='_'){
                    SystemCalluponCleanProgramExit[i]=' ';
                }
            }
        }
    }
